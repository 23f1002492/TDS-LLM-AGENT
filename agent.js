class ChatBox {
    ...
    async init() {
        try {
            ...
            console.log(`ChatBox v${this.version} initialized successfully`);
        } catch (error) {
    ...
    getDefaultSettings() {
        return {
            llm: { provider: 'aipipe', apiKey: '', model: 'default', maxTokens: 2000, temperature: 0.7, googleSearchApiKey: '', googleSearchEngineId: '' },
        ...
        return [
                ...
                function: { 
                    ...
                    parameters: { 
                        ...
                        properties: { 
                            ...
                            results: { type: "integer", default: 5, description: "Number of results" } 
                        }, 
                        required: ["query"] 
                    } 
                } 
            ...
            { 
                ...
                function: { 
                    ...
                    description: "Execute AI workflows using AI Pipe", 
                    parameters: { 
                        type: "object", 
                        properties: { 
    ...
    async executeTool(toolCall) {
        ...
        switch (name) {
            ...
            case 'aipipe_workflow': return await this.executeAIPipe(args);
    ...
    async executeWebSearch({ query, results = 5 }) {
        try {
            const apiKey = this.state.settings.llm.googleSearchApiKey;
            const searchEngineId = this.state.settings.llm.googleSearchEngineId;
            if (!apiKey || !searchEngineId) {
                return { error: "Google Search API key and Search Engine ID required in settings" };
            }
            
            const url = `https://www.googleapis.com/customsearch/v1?key=${apiKey}&cx=${searchEngineId}&q=${encodeURIComponent(query)}&num=${results}`;
    ...
    async executeCode({ code }) {
        try {
            // Simple sandboxing - capture console output
            ...
            try {
                ...
                // Try as statement if expression fails
                new Function(code)();
    ...
    async executeAIPipe({ workflow, data }) {
        try {
            const apiKey = this.state.settings.llm.apiKey;
            if (!apiKey) return { error: "AI Pipe token required" };
            
            const response = await fetch('https://aipipe.org/api/v1/workflow', {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) throw new Error(`AI Pipe error: ${response.status}`);
            const result = await response.json();
            
            return { workflow, result };
        } catch (error) {
            return { error: `AI Pipe failed: ${error.message}` };
        }
    }
    ...
    displayMessage(message) {
        try {
            ...
            const senderName = { user: 'You', assistant: 'ChatBox', system: 'System', tool: 'Tool' }[message.role] || message.role;
    ...
    editConversationName(conversationId) {
        ...
        const newName = prompt('Enter new conversation name:', conv.title);
        if (newName && newName.trim()) {
            ...
            this.updateChatHeader();
            this.saveCurrentConversation();
            this.showToast('success', 'Renamed', 'Conversation name updated.');
        }
    }
    ...
    deleteConversation(conversationId) {
        ...
        if (this.state.currentConversationId === conversationId) {
            const remaining = Array.from(this.state.conversations.values())
                .sort((a, b) => b.updatedAt - a.updatedAt)[0];
            
            if (remaining) {
                this.loadConversation(remaining.id);
            } else {
                this.createNewConversation();
            }
        }
        ...
        this.updateConversationList();
        this.saveCurrentConversation();
        this.showToast('success', 'Deleted', 'Conversation deleted.');
    }
    ...
    exportSettings() {
        try {
            const blob = new Blob([JSON.stringify(this.state.settings, null, 2)], { type: 'application/json' });
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url;
            a.download = 'chatbox-settings.json';
            ...
            this.showToast('error', 'Export Failed', e.message || 'Could not export settings');
        }
    }
        ...
        try {
            ...
            input.onchange = async (e) => {
                ...
                try {
                    ...
                    this.state.settings = { ...this.getDefaultSettings(), ...imported };
                    this.populateSettingsForm();
                    this.showToast('success', 'Imported', 'Settings imported successfully.');
                } catch (err) {
            ...
            this.showToast('error', 'Import Failed', e.message || 'Could not import settings');
        }
    }
    ...
    populateSettingsForm() {
        ...
        if (document.getElementById('google-search-key')) document.getElementById('google-search-key').value = s.llm.googleSearchApiKey || '';
        if (document.getElementById('google-search-engine-id')) document.getElementById('google-search-engine-id').value = s.llm.googleSearchEngineId || '';
    ...
    updateSettingsFromForm() {
        ...
        s.llm.googleSearchApiKey = document.getElementById('google-search-key')?.value || s.llm.googleSearchApiKey;
        s.llm.googleSearchEngineId = document.getElementById('google-search-engine-id')?.value || s.llm.googleSearchEngineId;
    ...
    updateConversationList() {
        try {
            ...
            this.elements.conversationList.innerHTML = conversations.map(conv => `
                <div class="conversation-item ${conv.id === this.state.currentConversationId ? 'active' : ''}" data-conversation-id="${conv.id}">
                    ...
                    <div class="conversation-actions">
                        <button class="btn btn-sm btn-outline-secondary edit-conv-btn" data-conversation-id="${conv.id}" title="Edit Name">
                            <i class="fas fa-edit"></i>
                        </button>
                        <button class="btn btn-sm btn-outline-danger delete-conv-btn" data-conversation-id="${conv.id}" title="Delete">
            ...
            // Add after the existing conversation item click listeners
            this.elements.conversationList.querySelectorAll('.edit-conv-btn').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    e.stopPropagation(); // Prevent conversation selection
                    this.editConversationName(btn.dataset.conversationId);
                });
            });
            this.elements.conversationList.querySelectorAll('.delete-conv-btn').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    e.stopPropagation(); // Prevent conversation selection
    ...
    showWelcomeMessage() {
        ...
        if (conv && conv.messages.length === 0) {
            this.addMessage('assistant', 'Welcome to ChatBox! How can I assist you?');
        }
    }
